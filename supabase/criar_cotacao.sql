CREATE OR REPLACE FUNCTION public.criar_cotacao(p_codigo_cotacao text, p_data_evento date, p_cidade text, p_time_option text, p_action_time integer, p_nome text DEFAULT NULL::text, p_telefone text DEFAULT NULL::text, p_quantidade_convidados integer DEFAULT NULL::integer)
 RETURNS TABLE(success boolean, message text, codigo_cotacao text, price_pb numeric, price_colorido numeric, guest_details text, route_details text, date_adjustment_details text)
 LANGUAGE plpgsql
AS $function$
DECLARE
    -- Variáveis de preço
...
BEGIN
    ...
    IF v_road_distance > 130 THEN
        RETURN QUERY SELECT false, 'Desculpe, para essa distância não conseguimos efetuar uma cotação automático. Clique aqui para ser direcionado para que façamos seu atendimento pelo WhatsApp.'::TEXT, NULL::TEXT, NULL::NUMERIC, NULL::NUMERIC, NULL::TEXT, NULL::TEXT, NULL::TEXT;
        RETURN;
    END IF;
    ...
    FROM pricing_data
    WHERE LOWER(TRIM(unaccent(city))) = LOWER(TRIM(unaccent(p_cidade)))
    AND type = 'PB' 
    AND time_hours = p_action_time
    LIMIT 1;
